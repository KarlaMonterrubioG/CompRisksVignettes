...
well as significance level from which we can infer that age, treatment given,
and clinical stage are significant. Note however that estimated hazard ratios are 
non-identical to those inferred in the cause-specific vignette. This is because
the risks set differ between these two hazard functions.
...
For instance, the sub-distribution hazard ratio of relapse is 1.363 when a
...
covariate values) at such event times. For instance, the cumulative incidence 
function of patient 1 (63 years, male, under radiation, no mediastinum 
involvement, no extranodal disease, and clinical stage 1) and patient 3 (27 
years, female, under chemotherapy treatment, no mediastinum involvement, no 
extranodal disease, and clinical stage 2) are shown below in red and blue, 
respectively. As an illustration, the following code is used to plot the 
estimated CIF for two patients. 
...
> **_NOTE:_** It is not straightforward to do predictions at a specific time
point using `cmprsk`. 
...
a specific time point (e.g. $t=30.5$) for a new dataset (e.g. `hd_test`). Below, 
...
CIF. However, it is not clear from the documentation how one can do predictions
at a new time point.
