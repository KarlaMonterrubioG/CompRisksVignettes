...
The first column in CIF_hat corresponds to the 72 unique event times and the
...
pred_DB <- predict.timereg(fit_DB, 
                           newdata = as.data.frame(hd_test[1:2,2:7]), se = 0)
plot(pred_DB,
     multiple = 1,
     col = c("red", "blue"),
     ylim = c(0, 0.57),
...
```{r}
pred_DB.t <- predict.timereg(fit_DB, newdata = hd_test[1:5, 2:7], times = 5)
c(pred_DB.t$P1)
```
...
$\alpha$ and $\lambda$ for patient 1, where we observe that the chains for $\alpha$
are not optimal. Note that in order to make valid inferences one should ensure convergence
was achieved.
...
pred_DDP <- predict(fit_DDP, newdata = as.data.frame(hd_test[c(1, 2), 2:7]))
```
...
pred_DDP.t <- predict(fit_DDP, 
                      newdata = as.data.frame(hd_test[1:5, 2:7]), tpred = 5)
pred_DDP.t$Fpred
```
