...
The first column in CIF_hat corresponds to the `r nrow(CIF_hat)` unique event times and the
remaining `r nrow(CIF_hat)-1` columns are the estimated CIFs for each patient (given their 
...
in clinical stage 1. The second patient is 41 years old, male, under radiation, 
...
`pseudo` contains the pseudo-values of the `r nrow(pseudo)` patients (rows) at the `r ncol(pseudo)` different
...
Note that `pred_DDP` contains posterior samples for the 2 subjects at `r ncol(pred_DDP$tpred)` observed
