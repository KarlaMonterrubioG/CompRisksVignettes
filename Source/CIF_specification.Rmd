...
The function `FGR()` in this R package acts as a wrapper of the `crr()` function described above.
...
```{r}
pred_DB.t <- predict.timereg(fit_DB, newdata = hd_test[, 2:7], times = 5)
pred_DB.t$P1[1:5]
```
...
Note that `pred_DDP` contains posterior samples for the 2 subjects at 
`r ncol(pred_DDP$tpred)` observed
...
pred_DDP.t <- predict(fit_DDP, 
                      newdata = as.data.frame(hd_test[, 2:7]), tpred = 5)
