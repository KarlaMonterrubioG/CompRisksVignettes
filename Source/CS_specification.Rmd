...
hd_train   <- as.data.frame(split_data$SAMP1)
hd_test    <- as.data.frame(split_data$SAMP2)
```
...
of interest (relapse). Second, it shows
the coefficients and hazard ratios (`exp(coef)`) for each covariate, along with
a significance test. The results suggest that age, treatment, and clinical stage 
are significantly associated with the hazard of relapse. For instance, being in 
clinical stage 2 increases the cause-specific hazard (rate) of relapse by 44.9%
...
Note that naively estimating survival/risk probabilities from this model, will result
...
(going from 23 to 43), increases the cause-specific hazard of relapse by 37.8%.
This is in contrast to the coefficients obtained by `print()`, which in this
case corresponds to $\exp(0.0159)= 1.0160$ and whose interpretation refers to
...
instance going from 30-40 years, i.e. a 10-year increase will result in a 17.2%
...
Assume that we are interested in making risk predictions at time $t=5$ years for a
new set of patients (i.e. the test dataset `hd_test`). As an illustration, we 
...
## The `CFC` package
The cause-specific framework for competing risks (CFC) implemented by 
@Mahari2019Bayesian permits to compute adjusted cause-specific CIFs through the 
`cfc.survreg` function. The package permits to fit both, parametric and 
nonparametric models (see `survreg.distributions` for available choices).


```{r, warning=FALSE}
require(CFC)

cfc.fit <- cfc.survreg(Surv(time, status) ~ age +
                        sex +
                        trtgiven +
                        medwidsi +
                        extranod +
...
The output of the fitted model shows degrees of freedom, deviance and $\lambda$ for the
iterations.
...
`mboost` permits cross-validation for hyper-parameter selection; specifically, the number of boosting iterations. Here, we show how to do this using 25 bootstrap samples (default). Estimation is done sequentially:
...
```
The above output shows that 543 boosting iterations is enough. Once the number is obtained one can use `glmboost` with the resulting number of boosting iterations.
