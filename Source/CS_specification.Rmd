...
hd_train   <- as.data.frame(split_data$SAMP1[,-1])
hd_test    <- as.data.frame(split_data$SAMP2[,-1])
```
...
(`exp(coef)`) for each covariate, along with a significance test. The results suggest that age, treatment, and clinical stage are significantly associated with the hazard of relapse. For instance, being in clinical stage 2 increases the cause-specific hazard (rate) of relapse by `r round((exp(coef(csh_survival)[7]) - 1)*100,2)`%
...
case corresponds to `exp(coef)`=`r round(exp(coef(csh_rms)[1]),2)` and whose interpretation refers to
...
increase in the cause-specific hazard of 17.21%, as shown below. Similarly, for discrete
...
stop.B 
```
The above output shows that `r stop.B` boosting iterations is enough. Once the number is obtained one can use `glmboost()` with the resulting number of boosting iterations.
