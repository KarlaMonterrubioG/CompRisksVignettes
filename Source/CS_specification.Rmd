...
```{r}
csh_mboost_best <- glmboost(Surv(time, status == 1) ~ age +
                         sex +
                         trtgiven +
                         medwidsi +
                         extranod +
                       ...
                       data = hd,
                       family = CoxPH(),
                       control = boost_control(mstop = stop.B,
                                               nu = 0.3,
                                               trace = FALSE),
                       center = FALSE)
```
...
coef.csh_survival <- summary(csh_survival)$coefficients[,c(1,3)]
# Method 2: csh_rms
coef.csh_rms <- 
aux1 <- data.frame(variable = rownames(summary(csh_survival)))
