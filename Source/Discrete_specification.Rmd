...
bart1 <- crisk2.bart(x.train = xtrain, 
                     ...
                     type = 'pbart',
                     ntree = 30, numcut = 100, 
                     ndpost = 500, nskip = 500, keepevery = 10, 
                     seed = 99)
```
...
when the number of unique event times is large. Another option would be to use
the `predict()` function. As seen below, this leads to similar numerical resuls
(the bigger discrepancies are likely due to lack of convergence). 

```{r}
pre <- surv.pre.bart(x.train=xtrain, x.test=xtrain,
                     times=hd_train$time, delta=hd_train$status)

check <- predict(bart1, newdata=pre$tx.test, newdata2=pre$tx.test)

smoothScatter(bart1.chains$surv.test.mean, check$surv.test.mean)
...
for(i in aux) {
  mycol <- mycol + 1
  # selects samples for individual i across all time-points
  post.mcmc <- bart1.chains$yhat.test[ , (i-1)*n.times + seq_len(n.times)]
  # calculates the geweke diagnostic
  z <- gewekediag(post.mcmc)$z
  # to set the limits in the plot below
  y <- max(c(4, abs(z)))
  ...
  ## plot the z scores vs. time for each patient
  if(i==aux[1]) plot(bart1.chains$times, z, ylim=c(-y, y), type='l',
                  xlab='t', ylab='z')
      else lines(bart1.chains$times, z, type='l', col = mycol)
  lines(bart1.chains$times, rep(1.96, n.times), type='l', lty=2)
  lines(bart1.chains$times, rep(-1.96, n.times), type='l', lty=2)
}
```
