...
(@HemantCompeting). The method can be fitted using the `randomForestSRC`
package. Below we showcase how to do that.
...
available and can be used to identify variables that are important for any
cause.
...
We can obtain the fitted trees; for instance, a plot of the fifth fitted tree is
shown:
...
`plot.competing.risk`. This command shows 3 plots: a CS cumulative hazard, the
CIF, and the conditional probability, stratified by event type. In the plot
...
corresponding to the $692$ subjects and in the columns the $136$ unique event
times.
...
var.mindepth <- var.select(rsf.lr,
  cause = 1,
  # is possible to select "vh.vimp" for variable importance measure
  method = "md",
  # level of conservativeness of the thresholding rule
  conservative = "medium"
)
...
`plot.competing.risk`, which shows 3 plots: a CS cumulative hazard, the CIF, and
the conditional probability, stratified by event type. In the plot below, the
event of interest is shown in black.
