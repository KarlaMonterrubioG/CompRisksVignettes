...
lines(test.lr$time.interest,   test.lr$cif[2,,1], col="blue")
legend("topright", legend = c("Patient 1", "Patient 2"),
       lty = c(1,1), col = c("red", "blue"))
```

The model permits to do predictions at observed unique event times. Below we 
show the predicted CIF for the first 5 patients in the test set at a specific 
time point $t=5$: 

```{r}
test.lr$cif[1:5, test.lr$time.interest==5, 1]
```

Note that these predictions are substantially different from those obtained by
other methods. 
...
pred_Others <- data.frame("testID" = seq_len(nrow(hd_test)),
                      "RF_logrank" = test.lr$cif[,test.lrCR$time.interest==5,1],
