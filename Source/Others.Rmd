...
`plot.competing.risk()`. This command shows averaged out-of-bag ensemble for: CS cumulative hazard, 
...
```{r}
obsv.times <- rsf.lr$time.interest  # unique event times
...
corresponding to the `r nrow(hd_train)` subjects and in the columns the 
`r length(obsv.times)` unique event times.
...
The predicted CIF for patient 1 (red) and 2 (blue) in the test set is shown:   

```{r, fig.dim=c(6,4)}
CIF.hat <- test.lr$cif[,,1]  # select event of interest k=1
...
lines(test.lr$time.interest,  CIF.hat[2,], col="blue")
legend("topright", legend = c("Patient 1", "Patient 2"),
       lty = c(1,1), col = c("red", "blue"))
```

The model permits to do predictions at observed unique event times. Below we show the predicted CIF for the first 5 patients in the test set at a specific time point $t=5$: 

```{r}
test.lr$cif[1:5,obsv.times==5,1]
```
...
```{r}
test.lrCR$cif[1:5,obsv.times==5,1]
```
...
```{r save_pred}
pred_Others <- data.frame("testID" = seq_len(nrow(hd_test)),
                      "RF_logrank" = test.lr$cif[,obsv.times==5,1],
                      "RF_logrankCR" = test.lrCR$cif[,obsv.times==5,1])
